...
export default function InvestorHome() {
  ...
  useEffect(() => {
    axios.get("http://127.0.0.1:8000/api/allprojectview")
      .then((response) => {
  ...
  const handleFilter = (status) => {
    setFilter(status);
    console.log("itahnnnnnnnnn", status)
  };

  const filteredProjects = filter === 'live'
  ? first.filter((project) => project.projectstatus === '0')
  : filter === 'completed'
  ? first.filter((project) => project.projectstatus === '1')
  : first;
  ...
  return (
    <div>
      ...
      <div className='filterbuttons'>
        <div
          className={`buttonone ${filter === 'all' ? 'active' : ''}`}
          onClick={() => handleFilter('all')}
        >
          All
        </div>
        <div
          className={`buttonone ${filter === 'live' ? 'active' : ''}`}
          onClick={() => handleFilter('live')}
        >
          Live
        </div>
        <div
          className={`buttonone ${filter === 'completed' ? 'active' : ''}`}
